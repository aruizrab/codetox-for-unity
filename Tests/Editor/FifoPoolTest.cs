...
public class FifoPoolTest
{
    ...
    {
        Assert.That(() => new FifoPool<TestClass>(null), Throws.ArgumentNullException);
    }    
    
    [Test]
    public void InitializePool_WithCapacityArgumentAsLessThanZero_ThrowsArgumentOutOfRangeException()
    {
        Assert.That(() => new FifoPool<TestClass>(_createObject, capacity: -1), Throws.Exception.TypeOf<ArgumentOutOfRangeException>());
    }      
    
    [Test]
    public void InitializePool_WithCapacityArgumentAsZero_ThrowsArgumentOutOfRangeException()
    {
        Assert.That(() => new FifoPool<TestClass>(_createObject, capacity: 0), Throws.Exception.TypeOf<ArgumentOutOfRangeException>());
    }  
    
    [Test]
    public void InitializePool_WithCreateObjectArgumentAsValid_DoesNotThrowException()
    {
        Assert.That(() => new FifoPool<TestClass>(_createObject), Throws.Nothing);
    } 
    
    [Test]
    public void InitializePool_WithDefaultArguments_PoolCapacityIsEqualTo10()
    {
        Assert.That(new FifoPool<TestClass>(_createObject).Capacity, Is.EqualTo(10));
    }
    
    [Test]
    public void InitializePool_WithCapacityArgumentAsCustomValue_PoolCapacityIsEqualToCustomValue()
    {
        Assert.That(new FifoPool<TestClass>(_createObject, capacity: 5).Capacity, Is.EqualTo(5));
    }
    
    [Test]
    public void InitializePool_WithDefaultArguments_PoolCountIsEqualToCapacity()
    {
        Assert.That(new FifoPool<TestClass>(_createObject).Count, Is.EqualTo(10));
    }
        
    [Test]
    public void InitializePool_WithCapacityArgumentAsCustomValue_PoolCountIsEqualToCapacity()
    {
        Assert.That(new FifoPool<TestClass>(_createObject, capacity: 5).Count, Is.EqualTo(5));
    }
        
    [Test]
    public void InitializePool_WithDefaultArguments_PoolIsFull()
    {
        Assert.That(new FifoPool<TestClass>(_createObject).IsFull, Is.True);
    }
        
    [Test]
    public void InitializePool_DefaultArguments_PoolIsNotEmpty()
    {
        Assert.That(new FifoPool<TestClass>(_createObject).IsEmpty, Is.False);
    }
        
    [Test]
    public void IsEmptyProperty_WithCountPropertyAsZero_ReturnsTrue()
    {
        var pool = new FifoPool<TestClass>(_createObject, capacity: 1);
        pool.Get();
    ...
    {
        var pool = new FifoPool<TestClass>(_createObject);
    ...
    {
        var pool = new FifoPool<TestClass>(_createObject, onGetObject: obj => obj.Number = 20);
        var obj = pool.Get();
    ...
    {
        var pool = new FifoPool<TestClass>(_createObject, onGetObject: obj => obj.Number = 20, capacity: 1);
        pool.Get();
        Assert.That(pool.IsEmpty, Is.True);
        var obj = pool.Get();
        Assert.That(obj.Number, Is.EqualTo(20));
    ...
    {
        var pool = new FifoPool<TestClass>(_createObject, onReturnObject: obj => obj.Number = 20);
        var obj = pool.Get();
        pool.Return(obj);
        Assert.That(obj.Number, Is.EqualTo(20));
    }
    ...
    {
        var pool = new FifoPool<TestClass>(_createObject, onRemoveObject: obj => obj.Number = 20, capacity: 3);
        var first = pool.Get();
        var second = pool.Get();
        var third = pool.Get();
    ...
    {
        var pool = new FifoPool<TestClass>(_createObject, capacity: 3);
    ...
    [Test]
    public void GetAndReturnPoolObjects_OrderIsFIFO()
    {
        var pool = new FifoPool<TestClass>(_createObject, capacity: 3);
